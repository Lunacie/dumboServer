...
SRCS		=		main.c						\
				...
				server/getClient.c				\
				server/writeResponse.c				\
				server/getPage.c				\
				server/cutPage.c				\
				server/exec.c					\
				server/getRequest.c				\
				server/destroyClient.c				\
				server/loadProjects.c				\
				server/parameters.c				\
				server/functions.c				\
				server/databases.c				\
				server/session.c				\
...
INCLUDES	=		$(addprefix -I./, $(shell find includes/ -type d)) -I/usr/include  

LDFLAGS		=		-L./libs/  -llncstring -llnclist -llncerror -L./usr/lib/x86_64-linux-gnu/ -ldl -luuid  -lcrypto
